# Frequently Asked Questions

In addition to the questions and answers below, be sure to check the [rulings](rulings.md) page and the [Advantages and Flaws](broken-reference) page.

### How do I spend XP?

{% content-ref url="experience-spending-and-timeskip.md" %}
[experience-spending-and-timeskip.md](experience-spending-and-timeskip.md)
{% endcontent-ref %}

### Scene Requests

_**"What kinds of things do I need to submit a scene request for?"**_ Any interaction with an SPC requires a scene request. Touchstones and any relationships gained through Backgrounds can be run by the player as a vignette. Care should be taken to not treat these relationships as tertiary characters. They should interact with other PCs as little as possible.

### Feeding and Bonds

#### _**"There are some areas with no difficulty noted. What do I use?"**_

Standard difficulty in V5 is three. Use that unless instructed otherwise.

#### _**"I got a messy critical/bestial failure on my hunting roll. What do I do?"**_

Type up the scene to the point of the roll and ping the staff role. We'll help you work out the specifics and tailor the results to the situation on a case-by-case scenario. Remember to try to reroll with Willpower using the "Avoid Messy" or "Reroll Failures" buttons before pinging staff.&#x20;

Keep in mind that there is a time-limit before the buttons get greyed out.

#### _**"How long do blood bonds last?"**_

Bonds last for one IC month per dot, up to a maximum of six. This is before taking any flaws or merits into account.

#### _**"Do Banu Haqim have to diablerize a kindred victim if they fail their Bane check?"**_

No. They will simply feed until their victim reaches Hunger 5 or they reach Hunger 1. You may always choose to diablerize an SPC, but check in to make sure diablerie is alright with your scene partner if a PC is your victim.

#### _**"How much can I feed from a mortal? What happens if I take more?"**_

A kindred can slake a maximum of 2 Hunger from a single mortal before endangering them. Each slake requires 3 days of recovery time. \
\
Unless they receive adequate medical treatment, a mortal who has had 3 or more Hunger taken from them must make a Strength + Stamina roll (DC equals the amount of Hunger slaked) to stay alive.

#### _**"How do PC mortals heal from feedings?"**_

Every point of Hunger slaked _after_ 2 equals one point of Aggravated damage; Three slaked is considered a harmful drink and requires medical attention. Harmful drinks (Core p. 212) count as Aggravated damage for each Hunger slaked.

Mortals can convert Aggravated damage each sunrise with a successful Intelligence + Medicine test (DC equals the total amount of Aggravated damage). Add +1 to the Difficulty if they are making the check themselves. The maximum points of Aggravated damage that a character can convert is half that character’s Medicine rating, rounded up. This cannot be done if their Superficial tracker is full. Aggravated damage does not heal on its own.

Mortals can remove a number of Superficial damage equal to their Stamina rating at sunrise.

#### _**"Help! I took too much from a mortal and they are dying!"**_

For mortals, a character with Medicine can convert Aggravated damage to Superficial damage as above. Get them to a Hospital even if healed by Obeah or Vicissitude.

### Weapons

#### _**"How do I obtain a firearm?"**_

Legally, you would need someone to go out during the day to file any paperwork and renew the license. The gun would not be in your character's name unless they were the one going out and doing those things themselves. A retainer can be used for this purpose. Resources are also required. Illegally, you would seek out appropriate criminal contacts and use them to get weapons. Resources would also be required for this method. The specific amount of dots needed depends on the weapon desired.

### <mark style="color:yellow;">Kindred Intimacy</mark>

#### _<mark style="color:yellow;">**"What is the pool I roll and what should the difficulty be?"**</mark>_

<mark style="color:yellow;">You don't, and there isn't one. To protect player agency, blood bonds this way are done entirely via RP and without dice involvement. If you want your character to be bound, then you can make that decision for yourself.</mark>

<mark style="color:yellow;">See the</mark> [<mark style="color:yellow;">blood bond homebrew</mark>](broken-reference) <mark style="color:yellow;">for more information on bonds. Kindred intimacy is all about applying the Kissing another to induce that euphoric pleasure (or not, if you're playing a Hecata). You can Kiss without drinking and it is up to the player what their character chooses to do. Some kindred can still have sex as normal, usually based on Humanity (Core p. 236-239) or specific Lore Sheets.</mark>

### Coteries and Domains

#### _**"How do I form a coterie?"**_

Gather the intended coterie members together, between 3-7 kindred. For coordination purposes, you may create a [group ticket](https://discord.gg/tFqVDZVmXn) and add the other members to it. From there, use the coterie creation rules (Core p. 195)

### Mask

#### _**"How do I make a Mask for someone?"**_

You need a 2 dot Mask in order to get the Cobbler merit at character creation. You need to gain 15 successes for the first dot and 30 successes for the second. Pools vary based on your character's methods. You can only work on one Mask at a time and doing so eats your project/training roll for the night. Once the Mask is finished, the recipient must pay for the Merit using experience by the end of the next time skip. Otherwise, it becomes unusable.

Common rolls for Mask making are Resolve + Larceny/Craft/Technology with relevant specialties added.

#### _"How do I get the Cobbler merit in RP?"_

To earn the Cobbler merit in RP, you need to be taught by another Cobbler and score 15 successes in a project, but you don't need to have a 2-dot Mask.

In character creation, the Cobbler merits still follows the rules as written_._&#x20;

### Using Recording Devices

Recording devices on your person must be marked in your header. You may review (in-character) up to three scenes per IC night. Each review consumes one of your hunting chances.

### Drugs

#### _"How can I find drugs?"_

To find various drugs, you must make a successful Charisma+Streetwise roll. The DC depends on the type of drug desired:

* **Alcohol:** DC 2
* **Cocaine/Meth/Speed:** DC 5
* **Hallucinogens:** DC 4
* **Heroin/Morphine/Oxy/Opiates:** DC 5
* **Marijuana:** DC 3

On a total failure, your character is poisoned.&#x20;

#### _"Can my Herd give me drug effects?"_

Yes. You forfeit your ability to gain Resonance from that Herd, but may select one type of drug to affect them instead. This can be changed at the same frequency as resonance for that level of herd.

### Thin-Blood Alchemy and Resonance

p.283 dictates: "The specific amount \[of resonant blood] needed \[for Thin-Blood Alchemy] depends on the degree of the donor’s Resonance and on the Storyteller’s whim."

We use the following guidlines:\
\
**Negligible:** No Resonance possible \
**Fleeting:** 2 Hunger's worth required \
**Intense:** 1 Hunger's worth required \
**Acute:** 1/2 Hunger's worth required

### The Player's Guide

#### _"Is X allowed?"_

[players-guide-material](../information-on-each-source-book/players-guide-material/ "mention")

#### _"Why is X banned?"_

Many items in the Player's Guide are unsuited for play-by-post. Some require too much ST fiat for a game with close to 100 characters; others are simply unbalanced for our style of play.

#### _"Why not homebrew something to fit better instead of banning it?"_

We try to keep powers and advantages as close to RAW as possible. Not only is this easier for us, but it's also easier for players who may come from or eventually join another game.

